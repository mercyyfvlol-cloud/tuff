...
def _create_tables(conn):
    ...
    """)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS indefinite_timeouts (
            guild_id INTEGER NOT NULL,
            user_id INTEGER NOT NULL,
            reason TEXT,
            PRIMARY KEY (guild_id, user_id)
...
# ---------- Indefinite timeouts ----------
# Discord's own timeout field caps out at 28 days -- there's no way to set a
# longer one via the API, ever. This table backs a renewal loop (see
# moderation.py) that keeps re-applying a fresh 28-day timeout well before
# the current one could expire, so it functions as indefinite in practice.

def add_indefinite_timeout(guild_id: int, user_id: int, reason: str):
    with get_conn() as conn:
        conn.execute("""
            INSERT INTO indefinite_timeouts (guild_id, user_id, reason) VALUES (?, ?, ?)
            ON CONFLICT(guild_id, user_id) DO UPDATE SET reason = excluded.reason
        """, (guild_id, user_id, reason))


def remove_indefinite_timeout(guild_id: int, user_id: int) -> bool:
    """Returns False if there wasn't one to remove."""
    with get_conn() as conn:
        cur = conn.execute("DELETE FROM indefinite_timeouts WHERE guild_id = ? AND user_id = ?", (guild_id, user_id))
        return cur.rowcount > 0


def is_indefinite_timeout(guild_id: int, user_id: int) -> bool:
    with get_conn() as conn:
        return conn.execute(
            "SELECT 1 FROM indefinite_timeouts WHERE guild_id = ? AND user_id = ?", (guild_id, user_id)
        ).fetchone() is not None


def get_all_indefinite_timeouts():
    with get_conn() as conn:
        return conn.execute("SELECT * FROM indefinite_timeouts").fetchall()


# ---------- Disabled commands ----------
# Global (bot-wide, not per-guild) kill switch -- lets the owner shut off a
# single misbehaving command (e.g. /play if ffmpeg breaks) without taking
# the whole bot down. Managed via /shutdown, see commandtoggle.py.

def disable_command(name: str, disabled_by: int):
    with get_conn() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO disabled_commands (command_name, disabled_by) VALUES (?, ?)", (name, disabled_by)
        )


def enable_command(name: str) -> bool:
    """Returns False if it wasn't disabled."""
    with get_conn() as conn:
        cur = conn.execute("DELETE FROM disabled_commands WHERE command_name = ?", (name,))
        return cur.rowcount > 0


def is_command_disabled(name: str) -> bool:
    with get_conn() as conn:
        return conn.execute("SELECT 1 FROM disabled_commands WHERE command_name = ?", (name,)).fetchone() is not None
