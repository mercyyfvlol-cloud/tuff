...
XP_EXCLUDED_CHANNEL_IDS = {1541766493258260550}
...
class Leveling(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        if message.author.bot or not message.guild:
            return
        if message.channel.id in XP_EXCLUDED_CHANNEL_IDS:
            return
    ...
    def _rank_embed(self, guild: discord.Guild, target: discord.Member) -> discord.Embed:
        ...
        boost_row = db.get_xp_boost(guild.id, target.id)
        if boost_row and boost_row["expires_at"]:
            remaining = datetime.fromisoformat(boost_row["expires_at"]) - datetime.now(timezone.utc)
    ...
    @app_commands.command(name="give_xp", description="[Admin] Hand someone some XP (only you see the confirmation)")
    ...
    async def give_xp_text(self, ctx: commands.Context, member: discord.Member, amount: int):
        ...
        if not 1 <= amount <= 1_000_000:
            await self._dm_or_warn(ctx, "Amount must be between 1 and 1,000,000.")
            return
        embed, level, new_level = await self._apply_xp_change(ctx.guild, member, amount, ctx.author)
        await self._dm_or_warn(ctx, embed=embed)
        if new_level > level:
            await self._announce_role_unlock(ctx.guild, member, new_level, ctx.channel)

    @app_commands.command(name="remove_xp", description="[Admin] Take XP away from someone (only you see the confirmation)")
    @app_commands.describe(member="Who", amount="How much")
    @app_commands.checks.has_permissions(manage_guild=True)
    ...
    async def _dm_or_warn(self, ctx: commands.Context, content: str = None, embed: discord.Embed = None):
        """DMs the confirmation instead of posting in-channel, so give_xp/remove_xp stay secret. Falls back to a self-deleting channel warning if DMs are closed."""
        try:
            await ctx.author.send(content=content, embed=embed)
        except discord.Forbidden:
            warning = await ctx.channel.send(f"{ctx.author.mention} I can't DM you the confirmation -- open your DMs to this server and try again.")
